...
class OwnerProcess extends Thread {
     ...
     */
    public void transfer_message_to_peer(Object msg) throws IOException {
     ...
     * @throws IOException if an I/O error occurs while reading stream header
     */
    public void transfer_message_to_peer(int msg) throws IOException {
     ...
     */
    private void reply_to_individual_peer_request(String message, int x)
            throws IOException,
        ...
        switch (message) {
            ...
            case Constants.NAME:
                // transmit file name
                transfer_message_to_peer((Object) this.file_name);
                break;
            case Constants.REQUEST:
                // Read block number as first input
                perform_request_operation(x);
                break;
            case Constants.DATA:
            ...
            case Constants.REGISTER:
                ...
                break;
            case Constants.PEER:
                perform_peer_operation();
                break;
            case Constants.CLOSE:
                this.perform_close_operation();
                return;
    ...
    private void perform_list_operation() throws IOException {
        ...
        transfer_message_to_peer(arrayList);
    }
     ...
     * @throws IOException if an I/O error occurs while reading stream header
     */
    private void perform_peer_operation()
            throws IOException {
        System.out.print(Constants.PEER_LIST);
        int peer_id = input_stream.readInt();
        for (int _peer : ServerDriver.list_peer.keySet()) {
        ...
        transfer_message_to_peer(ServerDriver.list_peer);
        int downLoad_neighbor_id =
                ServerDriver.master_DB.containsKey(download_port)
                        ? ServerDriver.master_DB.get(download_port) : 0;
        transfer_message_to_peer((Object) downLoad_neighbor_id);
        int upload_neighbor_id = getUploadNeighbor(peer_port);
        transfer_message_to_peer((Object) upload_neighbor_id);
    }
    ...
    private int getUploadNeighbor(int peer_port) {
        ...
        for (Map.Entry<Integer, PortDB> entry : ServerDriver.port_DB.entrySet()) {
            if (entry.getValue().get_download_port() == peer_port) {
                int peer_id = entry.getValue().get_peer_port();
                upload_neighbor_id = ServerDriver.master_DB.get(peer_id);
                break;
            }
        }
     ...
     * @throws IOException if an I/O error occurs while reading stream header
     */
    private void perform_close_operation()
            throws IOException {
        object_output_stream.close();
        input_stream.close();
        ServerDriver.list_peer.remove(clientId);
    }
     ...
     * @throws IOException if an I/O error occurs while reading stream header
     */
    private void perform_request_operation(int x)
            throws IOException {
        x = this.input_stream.readInt();
        transfer_message_to_peer(x);
        transfer_message_to_peer(this.current_block.get(x));
    }
     ...
     * @throws IOException if an I/O error occurs while reading stream header
     */
    private void perform_register_operation()
            throws IOException {
        int peer = ServerDriver.peer_id;
        ServerDriver.list_peer.put(ServerDriver.peer_id, ServerDriver.port);
        System.out.println(peer);
        System.out.println(ServerDriver.list_peer);
        int port = ServerDriver.port;
        System.out.println(port);
        transfer_message_to_peer(peer);
        transfer_message_to_peer(port);
    ...
    public void run() {
        while (true) {
            try {
                ...
                int x = -1;
                reply_to_individual_peer_request(message, x);
...
// download and upload ports for each peer
class PortDB {
    ...
    public int get_download_port() {
        return download_port;
    }

    public int get_peer_port() {
        return peer_port;
    }
}

// methods for ServerDriver
public class ServerDriver {
     ...
     * @param _file_name
     */
    public ServerDriver(int _owner_port, String _file_name) {
        ...
        try {
            while (true) {
                ...
                if (message_from_client.toUpperCase().startsWith(Constants.CONFIG)) {
                    ...
                    this.peer_id = id;
                    this.port = peer_port;
                    create_DB(id, peer_port, download_port);
                }
     ...
     */
    private void create_DB(int id, int peer_port, int download_port) {
        master_DB.put(peer_port, id);
        port_DB.put(id, new PortDB(peer_port, download_port));
    }

    // chunk division
    private void divide_file_into_chunks() {
        try {
            File directory_separate = new File(Constants.OWNER_DIR);
            if (!directory_separate.exists()) {
                directory_separate.mkdir();
            }
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 1) {
            ...
            new ServerDriver(owner_port, file_name).initiateOwner();
            // file_name = args[1];
